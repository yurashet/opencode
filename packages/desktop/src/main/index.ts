...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "by.softclub.mitracode.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `mitracode-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "MitraCode Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("mitracode://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("mitracode")
  registerRendererProtocol()
  setDockIcon()
